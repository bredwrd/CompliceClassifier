...
io.savemat('train_data.mat', {'train_task_descriptions': train_task_descriptions, 'train_task_labels': train_task_labels,
                              'train_wordcount_data': train_wordcount_data, 'train_besteffort_data': train_besteffort_data})
io.savemat('test_data.mat', {'test_task_descriptions': test_task_descriptions, 'test_task_labels': test_task_labels,
                             'test_wordcount_data': test_wordcount_data, 'test_besteffort_data': test_besteffort_data})

# Train network with dataset, for number of words in a task.
net = nl.net.newff([[min(min(test_wordcount_data), min(train_wordcount_data), 1), max(max(test_wordcount_data), max(train_wordcount_data))]], [1, 1])
